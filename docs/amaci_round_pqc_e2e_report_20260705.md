...
| 固定长度 authenticated decrypt array | [`crypto.rs`](../crates/proof-core/src/crypto.rs), [`process_messages.rs`](../crates/proof-core/src/circuits/process_messages.rs) | `decrypt_authenticated_array::<9>` 直接输出固定数组，并校验绑定 ML-KEM shared key、nonce、长度和密文 payload 的 HMAC-SHA256 tag；认证失败的队列消息被证明为 no-op，不会阻塞整个 batch。 | 降低 processMessages 热点路径里的分配和动态长度处理，同时拒绝密文或 tag 篡改及其批次 DoS。 |
